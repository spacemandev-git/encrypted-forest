...
The admin configures the size of the map (starting at 0,0 and going in all four directions until it hits the configured diameter). They also set the start and end time of the map. They also set the win condition type, which can be one of various types.
...
We’re going to setup the project as a monorepo with three components. They will be distributed in a monorepo, with `arcium init` for the on chain program, and `bunx sv create …` for the player client and `bun init` for the backend indexer.

**### Solana & Arcium Programs**
Using arcium init we’ll set up an arcium project. This will contain our on chain program. The program should be able to:

- Hard code an admin public key in constants
- Create a new ‘game’ instance, that has diameter, perlin noise seeds, diameter of map, game start time, end time, etc

...
/**
 * A locally-persisted discovered planet.
 */
export interface PersistedPlanet {
  /** hex-encoded planet hash (used as key) */
  hashHex: string;
  x: string; // bigint serialized as string
  y: string;
  gameId: string;
  hash: number[]; // Uint8Array serialized as number array
  keySeed: number[];
  bodyType: number;
  size: number;
  comets: number[];
  ...
  staticEncCiphertexts?: number[];  // 4 packed FE ciphertexts flattened (4 * 32 bytes)
  ...
  dynamicEncCiphertexts?: number[]; // 2 packed FE ciphertexts flattened (2 * 32 bytes)
}

/**
 * A locally-persisted decrypted event.
 */
...
/**
 * Player preferences stored locally.
 */
  ...
  if (!dbPromise) {
    dbPromise = openDB(DB_NAME, DB_VERSION, {
      upgrade(db) {
        ...
        // Scan progress (tracks explored regions)

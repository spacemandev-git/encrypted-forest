...
import { getRecentGames, removeRecentGame, type RecentGame } from "../game/history.js";
...
export default function GameSetup(props: GameSetupProps) {
  ...
  // Create state
  const [createGameId, setCreateGameId] = createSignal(
  ...
  const [hashRounds, setHashRounds] = createSignal(
    DEFAULT_HASH_ROUNDS.toString(),
  );
  const [winConditionType, setWinConditionType] = createSignal<
    "points" | "race"
  >("points");
  ...
  // Recent games for JOIN mode
  const [recentGames, setRecentGames] = createSignal<RecentGame[]>([]);

  // Load recent games when entering join mode or wallet changes
  createEffect(() => {
    const pk = walletPk();
    const m = mode();
    if (m === "join" && pk) {
      getRecentGames(pk).then(setRecentGames).catch(() => setRecentGames([]));
    }
  });

  function handleSelectRecent(game: RecentGame) {
    setJoinGameId(game.gameId);
    // Auto-submit after selecting
    handleJoin();
  }

  async function handleRemoveRecent(e: MouseEvent, game: RecentGame) {
    e.stopPropagation();
  ...
  async function handleJoin() {
    ...
    try {
      ...
      setJoinError("Game ID must be a number");
      return;
    }

    try {
      const activeWallet = props.walletStore.activeWallet();
      await props.session.joinGame(
        gid,
        props.walletStore.rpcUrl(),
        walletPk(),
  ...
  async function handleCreate() {
    ...
    let gid: bigint;
    try {
      gid = BigInt(createGameId().trim() || "1");
    } catch {
      setCreateError("Game ID must be a number");
      return;
    }

    let diameter: bigint;
    try {
      diameter = BigInt(mapDiameter().trim() || "100");
    } catch {
      setCreateError("Map diameter must be a number");
      return;
    }

    let speed: bigint;
    try {
      speed = BigInt(gameSpeed().trim() || "10000");
    } catch {
      setCreateError("Game speed must be a number");
      return;
    }
    ...
    if (winConditionType() === "points") {
      let ppm: bigint;
      try {
        ppm = BigInt(pointsPerMetal().trim() || "1");
      } catch {
        setCreateError("Points per metal must be a number");
        return;
      }
      ...
      let msd: bigint;
      try {
        msd = BigInt(minSpawnDistance().trim() || "10");
      } catch {
        setCreateError("Min spawn distance must be a number");
        return;
      }
    ...
    const activeWallet = props.walletStore.activeWallet();
    if (!activeWallet) {
      setCreateError("No wallet selected");
      return;
    }
    ...
    try {
      await props.session.createGame(
        args,
        props.walletStore.rpcUrl(),
        activeWallet.keypair,
      );
    ...
    >
      ...
      <Show when={mode() === "choose"}>
        ...
        >
          ...
          <div
            class={tui.dim}
            style={{ "border-top": "1px solid #333333", margin: "4px 0" }}
          />
        ...
        >
          ...
          <Show when={recentGames().length > 0}>
            <div style={{ display: "flex", "flex-direction": "column", gap: "4px" }}>
              <label class={tui.label}>RECENT GAMES</label>
              <For each={recentGames()}>
                {(game) => (
                  <button
                    class={tui.button}
                    onClick={() => handleSelectRecent(game)}
                    style={{
                      display: "flex",
                  ...
                  >
                    <span>
                      <span style={{ color: "#88ffbb" }}>#{game.gameId}</span>
                      <span class={tui.dim} style={{ "margin-left": "8px" }}>
                        {new Date(game.joinedAt).toLocaleDateString()}
                      </span>
                    </span>
                    <span
                      onClick={(e) => handleRemoveRecent(e, game)}
                      style={{
                        color: "#555555",
                        cursor: "pointer",
                        padding: "0 4px",
      ...
      <Show when={mode() === "create"}>
        ...
        >
          ...
          <div
            class={tui.dim}
            style={{ "border-top": "1px solid #333333", margin: "2px 0" }}
          />
            ...
            <div style={{ display: "flex", gap: "4px", "margin-top": "4px" }}>
              <button
                ...
                style={{
                  ...
                  padding: "2px 8px",
                  background:
                    winConditionType() === "points"
                      ? "rgba(153, 102, 255, 0.2)"
                      : undefined,
                  "border-color":
                    winConditionType() === "points" ? "#9966ff" : undefined,
                }}
              ...
              <button
                ...
                style={{
                  ...
                  padding: "2px 8px",
                  background:
                    winConditionType() === "race"
                      ? "rgba(153, 102, 255, 0.2)"
                      : undefined,
                  "border-color":
                    winConditionType() === "race" ? "#9966ff" : undefined,
                }}
          ...
          <Show when={winConditionType() === "points"}>
            <Field
              label="POINTS PER METAL"
              value={pointsPerMetal()}
              onInput={setPointsPerMetal}
              placeholder="1"
            />
          </Show>
          <Show when={winConditionType() === "race"}>
            <Field
              label="MIN SPAWN DISTANCE"
              value={minSpawnDistance()}
              onInput={setMinSpawnDistance}
              placeholder="10"
            />
          </Show>
